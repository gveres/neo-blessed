...
function List(options) {
  ...
  if (options.items) {
    this.ritems = options.items.map(x => x['content'] || x);
...
List.prototype.createItem = function(content, meta = undefined) {
  var self = this;
...
List.prototype.appendItem = function(content, meta = undefined) {
  metaData = content['metaData'] || meta;
  content = content['content'] || (typeof content === 'string' ? content : content.getContent());

  var item = this.createItem(content, meta);
...
List.prototype.insertItem = function(child, content) {
  ...
  var item = this.createItem(content, metaData);
  for (var j = i; j < this.items.length; j++) {
    this.items[j].position.top++;
  }
...
List.prototype.setItems = function(items) {
  ...
  this.ritems = items.map(x => x['content'] || x);
